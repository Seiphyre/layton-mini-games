...
public class BoardView_OLD : MonoBehaviour
{
    [Header("Board"), Space]
    ...
    public int GridColCount => m_BoardData.Width;
    public int GridRowCount => m_BoardData.Height;
    ...
    private GameObject CreateLayer(string layerName, VisualTile cellTemplate, Transform parent)
    {
        ...
        if (cellTemplate != null)
        {
            for (int rowIndex = 0; rowIndex < m_BoardData.Height; rowIndex++)
            {
    ...
    private GameObject CreateRow(string rowName, VisualTile cellTemplate, Transform parent, int rowIndex)
    {
        ...
        for (int colIndex = 0; colIndex < m_BoardData.Width; colIndex++)
        {
    ...
    private void CreateDropPreview(PieceDefinition piece)
    {
    ...
    public VisualBoardElement CreatePiece(PieceDefinition shopItem)
    {
        VisualBoardElement piece = Instantiate(m_PieceTemplate, m_PiecesLayer.transform);
    ...
    private bool IsOutOfBound(VisualBoardElement piece)
    {
        return piece.Position.x < 0
            || piece.Position.y < 0
            || piece.Position.x + piece.Size.x > m_BoardData.Width
            || piece.Position.y + piece.Size.y > m_BoardData.Height;
    }
    ...
    private bool CollideWithWall(VisualBoardElement piece)
    {
        ...
        //        if (tile != null)
        //        {
        //            if (tile.Walls.North && y != min.y) // Collision with north wall
        //                return true;

        //            if (tile.Walls.South && y != max.y) // Collision with south wall
        //                return true;

        //            if (tile.Walls.West && x != min.x) // Collision with west wall
        //                return true;

        //            if (tile.Walls.East && x != max.x) // Collision with east wall
        //                return true;
        //        }
        //    }
        //}
